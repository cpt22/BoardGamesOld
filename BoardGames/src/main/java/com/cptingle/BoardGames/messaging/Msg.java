...
public enum Msg {
    ...
    ARENA_AUTO_START("Game will auto-start in &c%&r seconds."),
    ARENA_START_DELAY("Game can start in &e%&r seconds."),
    JOIN_NOT_ENABLED("BoardGames is not enabled."),
    JOIN_IN_OTHER_ARENA("You are already in an game! Leave that one first."),
    JOIN_ARENA_NOT_ENABLED("This game is not enabled."),
    JOIN_ARENA_NOT_SETUP("This game has not been set up yet."),
    ...
    JOIN_ARENA_IS_RUNNING("This game is already in progress."),
    JOIN_ALREADY_PLAYING("You are already playing!"),
    JOIN_ARG_NEEDED("You must specify an game."),
    JOIN_NO_PERMISSION("You don't have permission to join any games."),
    ...
    JOIN_PLAYER_JOINED("You joined the game. Have fun!"),
    LEAVE_NOT_PLAYING("You are not in the game."),
    LEAVE_NOT_READY("You did not ready up in time! Next time, ready up by clicking an iron block."),
    LEAVE_PLAYER_LEFT("You left the game. Thanks for playing!"),
    ...
    SPEC_NOT_RUNNING("This game isn't running."),
    SPEC_EMPTY_INV("Empty your inventory first!"),
    SPEC_ALREADY_PLAYING("Can't spectate when in the game!"),
    NOT_READY_PLAYERS("Not ready: &c%&r"),
    FORCE_START_RUNNING("Game has already started."),
    FORCE_START_NOT_READY("Can't force start, no players are ready."),
    FORCE_START_STARTED("Forced game start."),
    FORCE_END_EMPTY("No one is in the game."),
    FORCE_END_ENDED("Forced game end."),
    ...
    WARP_TO_ARENA("Warping to the game not allowed!"),
    WARP_FROM_ARENA("Warping from the game not allowed!"),
    ...
    PLAYER_LEAVE("You left the game!"),
    PLAYER_NOT_IN_GAME("You are not currently in a game!"),
    
    GAME_YOUR_TURN("It is your turn!"),
    ...
    MISC_LIST_GAMES("Available games: %"),
    ...
    MISC_MA_LEAVE_REMINDER("Remember to use &e/bg leave&r when you are done."),
    MISC_NONE("&6<none>&r");
    ...
    static void load(ConfigurationSection config) {
        for (Msg msg : values()) {
            // ARENA_END_GLOBAL => game-end-global
