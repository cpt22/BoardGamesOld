...
public class Commands {
	 ...
	 *
	 * @param sender
	 *            a CommandSender reference, possibly a proxy, non-null
	...
	public static Game getGameToJoin(GameMaster gm, Player p, String arg1) {
        // Check if BoardGames is enabled first.
        if (!gm.isEnabled()) {
        ...
        if (arg1 != null) {
            ...
            if (games.size() > 1) {
               ...
               //gm.getGlobalMessenger().tell(p, Msg.MISC_LIST_GAMES.format(ECUtils.listToString(games)));
                return null;
            }
            game = games.get(0);
        }
